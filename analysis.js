...
async function init(context, scope) {
  ...
  const payload = scope.find(x => x.variable === 'form_payload') || { value: env.payload, device: env.device_id };
  const port = scope.find(x => x.variable === 'form_port') || { value: env.default_PORT };
  if (!payload || !payload.value || !payload.device) return context.log('Missing "form_payload" in the data scope.');
  ...
  if (!device_id) return context.log('Device parameter <device> not found in the variables sent by the widget/dashboard.');
